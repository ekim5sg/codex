...
impl Default for AppState {
    fn default() -> Self {
        Self {
            ...
            cloudflare_worker_url: String::new(),
...
fn cloudflare_ready(state: &AppState) -> bool {
    !state.cloudflare_worker_url.trim().is_empty() && !state.cloudflare_api_key.trim().is_empty()
}

fn push_daily_record(state: &AppState, status: UseStateHandle<String>) {
    if !cloudflare_ready(state) {
        return;
    }
    ...
    let worker = state
        ...
        .trim_end_matches('/')
        .to_string();
    let key = state.cloudflare_api_key.clone();
    let payload = DailyRecordPayload {
        day_key: state.day_key.clone(),
        total_oz: state.total_oz,
        entries: state.entries.clone(),
        symptoms: state.symptoms.clone(),
        updated_at_ms: now_ms(),
    };

    status.set("Syncing to Cloudflare…".to_string());
    spawn_local(async move {
        let resp = Request::post(&format!("{worker}/records"))
        ...
        match resp {
            Ok(req) => match req.send().await {
                Ok(r) if r.ok() => status.set("Cloudflare sync complete.".to_string()),
                Ok(r) => status.set(format!("Cloudflare sync failed: HTTP {}", r.status())),
                Err(_) => status.set("Cloudflare sync failed: network error.".to_string()),
            },
            Err(_) => {
                status.set("Cloudflare sync failed: could not serialize payload.".to_string())
            }
        }
    });
}

fn fetch_weekly_report(
    state: &AppState,
    weekly: UseStateHandle<WeeklyReport>,
    status: UseStateHandle<String>,
) {
    if !cloudflare_ready(state) {
        status.set("Add Cloudflare Worker URL and API key first.".to_string());
        return;
    }
    ...
    status.set("Loading weekly report…".to_string());
    spawn_local(async move {
        let resp = Request::get(&format!("{worker}/reports/weekly?end_day={today}"))
            .header("x-api-key", &key)
            .send()
            .await;

        match resp {
            Ok(r) if r.ok() => match r.json::<WeeklyReport>().await {
                Ok(report) => {
                    weekly.set(report);
                    status.set("Weekly report updated.".to_string());
                }
                Err(_) => status.set("Weekly report parse failed.".to_string()),
            },
            Ok(r) => status.set(format!("Weekly report failed: HTTP {}", r.status())),
            Err(_) => status.set("Weekly report failed: network error.".to_string()),
        }
    });
}
...
fn app() -> Html {
    ...
    let weekly_report = use_state(WeeklyReport::default);
    let sync_status = use_state(|| "Cloudflare sync not configured yet.".to_string());
    ...
    let add_oz = {
        ...
        Callback::from(move |oz: u32| {
            ...
            save_state(&next);
            push_daily_record(&next, sync_status.clone());
            state.set(next);
        })
    };
    ...
    let set_worker_url = {
        ...
        Callback::from(move |e: InputEvent| {
            let input: HtmlInputElement = e.target_unchecked_into();
            let mut next = load_current_or_default();
            next.cloudflare_worker_url = input.value();
            save_state(&next);
            state.set(next);
        })
    };
    ...
    let set_worker_key = {
        ...
        Callback::from(move |e: InputEvent| {
            let input: HtmlInputElement = e.target_unchecked_into();
            let mut next = load_current_or_default();
            next.cloudflare_api_key = input.value();
            save_state(&next);
            state.set(next);
        })
    };
    ...
    let sync_now = {
        ...
        let sync_status = sync_status.clone();
        Callback::from(move |_| push_daily_record(&current, sync_status.clone()))
    };

    let load_weekly = {
        let current = (*state).clone();
        let weekly_report = weekly_report.clone();
        let sync_status = sync_status.clone();
        Callback::from(move |_| {
            fetch_weekly_report(&current, weekly_report.clone(), sync_status.clone())
        })
    };
    ...
    html! {
        <main class="app-shell">
                ...
                <aside class="stack">
                    <section class="card">
                        ...
                        <div class="stack">
                            ...
                            <hr class="sep" />
                            <h4>{"Cloudflare Sync"}</h4>
                            <div>
                                <div class="label">{"Worker URL"}</div>
                                <input class="input" type="text" value={state.cloudflare_worker_url.clone()} oninput={set_worker_url}/>
                            </div>
                            <div>
                                <div class="label">{"API Key"}</div>
                                <input class="input" type="password" value={state.cloudflare_api_key.clone()} oninput={set_worker_key}/>
                            </div>
                            <div class="row">
                                <button class="btn primary" onclick={sync_now}>{"Sync Today"}</button>
                                <button class="btn good" onclick={load_weekly}>{"Load Weekly Report"}</button>
                            </div>
                            <div class="small muted">{(*sync_status).clone()}</div>
            ...
            <div class="grid" style="margin-top:16px;">
                ...
                <section class="card">
                    <h3>{"Weekly Report (Cloudflare)"}</h3>
                    <div class="history-list">
                        {
                            if weekly_report.days.is_empty() {
                                html! { <div class="muted">{"No weekly data loaded yet."}</div> }
                            } else {
                                html! {
                                    <>
                                        { for weekly_report.days.iter().map(|d| html! {
                                            <div class="history-item">
                                                <span>{d.day_key.clone()}</span>
                                                <span class="muted">{format!("{} oz", d.total_oz)}</span>
                                            </div>
                                        }) }
                                        <hr class="sep"/>
                                        <div class="history-item"><span>{"7-day total"}</span><span>{format!("{} oz", weekly_report.total_oz)}</span></div>
                                        <div class="history-item"><span>{"Daily average"}</span><span>{format!("{:.1} oz", weekly_report.average_oz)}</span></div>
                                    </>
                                }
                            }
                        }
